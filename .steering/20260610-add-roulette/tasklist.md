...
## フェーズ6: 動作確認後の追加要求対応（2026-06-10）

ユッカによる実機・手動の動作確認は全観点OK。その際の追加要求3件に対応する。

- [x] 当選オーバーレイの背景（カード外側）クリックでも「もう一度」と同じリセット挙動にする
  - [x] `RouletteView.bindEvents` に背景クリック→`onReset` を追加（カード内クリックは対象外）
  - [x] ユニットテスト追加（背景クリックで発火・カード内クリックで非発火）
- [x] Stop直後に加速して見える違和感を解消する（回転スピードの連続性）
  - [x] 減速の所要時間を「総距離と等速スピード」から逆算し、減速の初速＝等速回転スピードにする
  - [x] 等速回転のスピードを少し速くする（0.36 → 0.5 deg/ms）
  - [x] ユニットテスト更新（速度定数）＋速度連続性のテスト追加
- [x] ホイール円周の茶色の縁を細くする（lineWidth 6 → 3）
- [x] 再調整（2026-06-11）: 等速回転を1000度/秒へ引き上げ（減速時間は約1.8〜3.6秒に短縮）
- [x] 再調整（2026-06-11）: 外周ラインを茶色→グレー（#9e9e9e）・lineWidth 2 に変更
- [x] 再調整（2026-06-11）: 減速時間を約10秒に延長（最低周回数 1→5周。速度連続性の式は維持）
- [x] 再調整（2026-06-11）: 区画の色をお店に安定割当（「もう一度」で色が変わる問題を修正。隣接同色の回避は色変更ではなく再シャッフルで実施）
- [x] 再調整（2026-06-11）: 色パレットを5色→10色に拡張（明るめトーン・隣接色相が近くならない並び順）
- [x] ドキュメント更新（PRD実機確認2項目のチェック・functional-design.md A-3/A-4）
- [x] 品質チェック（test / lint / typecheck / build）（125テスト・全コマンド成功）

> **設計変更メモ**: 減速時間は当初「4000〜6000msの乱数」だったが、速度連続性の要求により「総距離（最低1周＋着地点まで=360〜720度）÷等速スピードから逆算（約3.6〜7.2秒）」へ変更。余分回転4〜6周の仕様は廃止（easeOutQuintの初速は平均速度の5倍のため、初速を等速と一致させると周回数は必然的に1〜2周になる）。当選の完全ランダム性（着地点を先に乱数で決める方式）は維持。
