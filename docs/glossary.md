...
### 未検出エラー (NotFoundError)

**クラス名**: `NotFoundError`

**継承元**: `Error`（`id` フィールドを保持）

**発生条件**: 存在しないお店の `id` に対する更新・削除・対象切替（一覧の競合などで対象が消えた場合）。

**対処方法**:
- ユーザー: 一覧を再読み込みして最新状態を確認
- 開発者: 操作対象の `id` の整合性・呼び出し経路を確認
...
if (index === -1) {
  throw new NotFoundError(id); // メッセージ既定値: 「お店が見つかりません」
}
```
