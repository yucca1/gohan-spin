 ...
 * @throws {StorageError} localStorageの読み書きに失敗した場合
 *
 * @example
 * ```typescript
...
```typescript
// ブラウザSPA(サーバーレス)では、そもそもフロントにシークレットを持たない。
// バンドルされたJSは誰でも閲覧できるため、APIキー等を埋め込んではいけない。

// ❌ 悪い例: ハードコード(ビルド成果物に含まれ、誰でも読めてしまう)
const apiKey = 'sk-1234567890abcdef'; // 絶対にしない！

// ✅ 良い例: 機密が必要な処理はサーバー側に置き、フロントは鍵を持たない。
//   本アプリのように外部API連携がない場合は、そもそもシークレットを扱わない設計にする。
```
...
- [ ] 入力検証が実装されている
- [ ] 機密情報がハードコードされていない
- [ ] XSS対策(textContentの使用、入力のサニタイズ)がされている
