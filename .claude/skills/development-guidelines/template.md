...
**例**:
```typescript
// E2EはPlaywright等でブラウザのDOM操作を検証する
describe('アイテム追加フロー', () => {
  it('ユーザーがアイテムを追加すると一覧に表示される', async () => {
    // 追加フォームに入力して送信
    await page.fill('#item-name', '新しいアイテム');
    await page.click('#add-button');

    // 一覧に反映されることを検証
    await expect(page.locator('.item-list')).toContainText('新しいアイテム');

    // リロードしても保持される(localStorage永続化の確認)
    await page.reload();
    await expect(page.locator('.item-list')).toContainText('新しいアイテム');
  });
});
...
- [ ] 不要な計算がないか
- [ ] メモリリークの可能性がないか
- [ ] localStorageアクセスや再描画(DOM操作)が最適化されているか
