...
/** 等速回転の速度（度/ミリ秒）。0.36 = 1秒で360度（実機で微調整可）。 */
const SPIN_SPEED_DEG_PER_MS = 0.36;
/** 減速時の余分回転数の下限（周）。数周回してから止めることで自然な減速に見せる。 */
const MIN_EXTRA_TURNS = 4;
/** 減速時の余分回転数の上限に加わるランダム幅（周）。4〜6周になる。 */
const EXTRA_TURNS_RANDOM_RANGE = 3;
/** 減速の最短時間（ミリ秒）。 */
const MIN_DECELERATION_MS = 4000;
/** 減速時間に加わるランダム幅（ミリ秒）。4000〜6000msになる。 */
const DECELERATION_RANDOM_RANGE_MS = 2000;

/**
 * 区画の塗り色パレット。
 * 円環の隣接（先頭と末尾も隣接する）で同色が並ばないよう、4色以上を用意して循環割当する。
 */
...
export class RouletteEngine {
  ...
  build(enabledShops: Shop[]): WheelSegment[] {
    ...
    const shuffled = shuffle(enabledShops);
    ...
    this.segments = shuffled.map((shop, i) => ({
      ...
      color: pickColor(i, shuffled.length),
    }));
  ...
  stop(onUpdate: AngleListener, onFinish: (winner: Shop) => void): void {
    ...
    const extraTurns =
      MIN_EXTRA_TURNS + Math.floor(Math.random() * EXTRA_TURNS_RANDOM_RANGE);
    const landing = Math.random() * 360; // 着地点（完全ランダム＝抽選の本体）
    const to = from + extraTurns * 360 + landing;
    const durationMs =
      MIN_DECELERATION_MS + Math.random() * DECELERATION_RANDOM_RANGE_MS;
...
/**
 * i 番目の区画の色を決める。基本はパレットの循環割当。
 * 末尾区画がパレット循環で先頭区画と同色になる場合（N % palette = 1 など）は
 * 次の色へずらし、円環の先頭・末尾が同色で隣接するのを防ぐ。
 */
function pickColor(i: number, total: number): string {
  const palette = SEGMENT_PALETTE;
  let color = palette[i % palette.length];
  const isLast = i === total - 1;
  if (isLast && total > 1 && color === palette[0]) {
